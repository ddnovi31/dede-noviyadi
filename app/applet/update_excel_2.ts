...
code = code.replace(
  /earthWtCol = pushCol\(item\.result\.bom\.earthingConductorWeight \|\| 0, fmtNum\);/g,
  `earthWtCol = pushCol(null, fmtNum, \`PI()*(\${getColName(colIdx - 2)}\${r}/2)^2*\${getColName(colIdx - 3)}\${r}*\${getDensity('Cu')}\`);`
);
// Wait, for NFA2X-T, it's different. Let's just replace the whole Earth block.
code = code.replace(
...
// And remove Total Price from the row data
code = code.replace(
